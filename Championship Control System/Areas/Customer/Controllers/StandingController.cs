...
namespace Championship_Control_System.Areas.Customer.Controllers
{
    ...
    public class StandingController : Controller
    {
        ...
        {
            var championships = await _championshipRepo.GetAsync(include: q => q.Include(c => c.TeamStandings).ThenInclude(ts => ts.Team), cancellationToken: cancellationToken);
        ...
        {
            var championship = await _championshipRepo.GetOneAsync(c => c.ChampionshipId == id, include: q => q.Include(c => c.TeamStandings).ThenInclude(ts => ts.Team),cancellationToken: cancellationToken);
